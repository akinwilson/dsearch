...
class MeiliClientIndexer:
    ...
    def meili_engine_initialisation(self):
        ...
        print(f"health status after waiting for {WAIT}s :", client.health()["status"])
...
def handler(event, lambda_context):
    ...
    indexes = ["user", "content", "session", "entire"]
    ...
    convertor = DB2Json()
    f = time.time()
    print(f"DB2Json().__init__(): {f - s} seconds")
    indexer = MeiliClientIndexer()
    for INDEX_NAME in indexes:
        s = time.time()
        input_json = convertor.df_to_json(INDEX_NAME)
        f = time.time()
        print(f"DB2Json().df_to_json({INDEX_NAME}): {f - s} seconds")
        # Loading configuration file for index
        try:
            config_file_path = [file for file in config_files if INDEX_NAME in file][0]
        except IndexError:
            print("cannot find configuration files")
            pass
        print("prining config_file_path:\n", config_file_path)
        with open(config_file_path, "r") as file:
            config = json.loads(file.read())
        print("*" * 20)
        print("prining config type:\n", type(config))
        print("*" * 20)
        print("INDEX_NAME var: ", INDEX_NAME)
        # index creation
        ...
        f = time.time()
        print(f"MeiliClientIndexer().indexing('item',{INDEX_NAME}): {f - s} seconds")

    ##################################################################################
    # print("The current dirctory is: ", os.getcwd())
    # print("The content of the cwd is: ", os.listdir())
    # print(f"The contents of the directory {DOCKER_DB_PATH}, the directory inside the container, is: ", os.listdir("/var/task/mnt/efs"))
    ##################################################################################
    print(
        f"The contents of the directory provided to the meili engine, {DB_PATH}, is: ",
        os.listdir(DB_PATH),
    )
