...
###########################
MNT_PATH = os.environ["MEILI_DB_PATH"]
###########################
...
class Preprocessor:
    def __init__(self):
        ...
        self.total_records = {
            k: self.df_to_records(v) for k, v in self.total_dict.items()
        }
    ...
    def question_formatting(self):
        ...
        def profile_formatting(q):
            ...
            q.a_profile_url = q.a_profile_url.str.split("-").apply(
                lambda x: " ".join(x)
            )
        ...
        dfq = (
            q_text_formatting(tags_formatting(profile_formatting(q)))[
                ["profile", "tags", "question"]
            ]
    ...
    def user_formatting(self):
        ...
        def topic_formatting(u):
            u.topic_url = u.topic_url.apply(
                lambda x: [y.split("topic/")[-1] for y in x]
            )
            u.topic_url = u.topic_url.apply(
                lambda x: [str(re.sub("-", " ", y)) for y in x]
            )
            u.topic_url = u.topic_url.apply(lambda x: [y for y in x if y != "None"])
            u.rename(columns={"topic_url": "topic"}, inplace=True)
            return u

        dfu = topic_formatting(bio_formatting(profile_formatting(u)))[
            ["profile", "bio", "topic"]
        ]
    ...
    def primary_id_formatting(self):
        function_dict = {"user": self.dfu, "question": self.dfq}
        for (k, v) in function_dict.items():
            function_dict[k]["id"] = function_dict[k].id.apply(
                lambda i: str(i) + "_" + k
            )
...
class MeiliClientIndexer:
    def __init__(self):
        ...
        #     ["/usr/bin/meilisearch", "--db-path", MNT_PATH], shell=True
        # )

        self.process = subprocess.Popen(["/usr/bin/meilisearch"], shell=True)
    ...
    def health_check(self):
        ...
        logger.info(
            f"health status after waiting for {WAIT}s : {client.health()['status']}"
        )
...
def handler(event, lambda_context):
    ...
    f = time.time()
    logger.info(f"DB2Json().__init__(): {f - s} seconds")
    ...
    for INDEX_NAME in indexes:
        ...
        f = time.time()
        logger.info(f"DB2Json().df_to_json({INDEX_NAME}): {f - s} seconds")
        # Loading configuration file for index
        try:
            config_file_path = [file for file in config_files if INDEX_NAME in file][0]
        except IndexError:
            logger.info("cannot find configuration files")
            pass
        logger.info(f"prining config_file_path: {config_file_path}")
        ...
        logger.info(f"Response following indexing: {r.__dict__}")
        s = time.time()
        indexer.indexing(index=INDEX_NAME, json_records=input_json)
        f = time.time()
        logger.info(
            f"MeiliClientIndexer().indexing('item',{INDEX_NAME}): {f - s} seconds"
        )
    indexer.process.kill()
    logger.info(
        f"Co    ntents dir provided to meili engine, --db {MNT_PATH}, is: {os.listdir(MNT_PATH)}"
    )
    for ext in os.listdir(MNT_PATH):
        logger.info(
            f"The contents of the directory extension with {os.path.join(MNT_PATH, ext)}, is: {os.listdir(os.path.join(MNT_PATH, ext))}"
        )
    return {
        "statusCode": 200,
